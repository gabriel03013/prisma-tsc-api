...
export async function getAllUsersHandler(req: FastifyRequest, res: FastifyReply, include: boolean = false): Promise<User[]> {
    return res.send(await getAllUsers(include));
}

export async function getUserByIdHandler(req: FastifyRequest, res: FastifyReply, id: number, include: boolean = false): Promise<User> {
    return res.send(await getUserById(id, include))
}

export async function getUserByNameHandler(req: FastifyRequest, res: FastifyReply, name: string, include: boolean = false) : Promise<User[]> {
